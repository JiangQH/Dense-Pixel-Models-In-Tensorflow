    ...
    with tf.Graph().as_default() as g:
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            for step in xrange(config.max_iter + 1):
                ...
                _, loss_val, accu = sess.run([train_op, loss, accuracy], feed_dict=train_feed_dict)
                if step % config.display == 0 or step == config.max_iter:
                    print '{}[iterations], time consumes {}, train loss {}, accuracy {}'.format(step,
            ...
            for step in xrange(5):
                imgs, gts = data_loader.next_train_batch()
                feed_dict = {images: imgs, is_training: False}
                pres = predictions.eval(session=sess, feed_dict=feed_dict)
                for i in range(len(gts)):
                    pre = pres[i, ...]
                    gt = gts[i, ...]
                    scipy.misc.imsave(osp.join(outs, str(step)+str(i)+'_pre.png'), pre)
                    scipy.misc.imsave(osp.join(outs, str(step)+str(i)+'_gt.png'), gt)
